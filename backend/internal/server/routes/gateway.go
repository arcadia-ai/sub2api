...
func RegisterGatewayRoutes(
	...
	conversationCapture := middleware.ConversationCapture(h.Conversation)
	...
	gateway.Use(conversationCapture)
	...
	gemini.Use(conversationCapture)
	...
	r.POST("/responses", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationCapture, compositeTarget, requireGroupAnthropic, responsesHandler)
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationCapture, compositeTarget, requireGroupAnthropic)
	{
	...
	r.POST("/chat/completions", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), conversationCapture, compositeTarget, requireGroupAnthropic, func(c *gin.Context) {
		if isOpenAIResponsesCompatibleGatewayPlatform(c) {
			h.OpenAIGateway.ChatCompletions(c)
			return
		}
